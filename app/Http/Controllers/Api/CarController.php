...
//                return  $user->favourite_car;
    ...
    public function car_filter(Request $request)
    {
        $rule = [
            'name' => 'nullable',
        ...
        if ( isset($request->category_id) && trim($request->category_id !== '') ) {
            $car->where('category_id', '=', trim($request->category_id));
        }
        if ( isset($request->sub_category_id ) && trim($request->sub_category_id  !== '') ) {
            $car->where('sub_category_id', '=', trim($request->sub_category_id ));
        }
        ...
        if ( isset($request->rate) && trim($request->rate !== '') ) {
            $car= $car->get()->filter(function($item) use ($rate) {
        ...
        if ($car) {
            ...
            return $this->respondSuccessPaginate1($cars, trans('message.data retrieved successfully.'));
